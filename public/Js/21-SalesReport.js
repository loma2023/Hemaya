...
function ShowGeneralDataOrExpensesReceipts(MainData, Decode) {
    ...
    let Status = "تأجير";
    if (Title.includes("استئجأر")) { Status = "استئجأر" }
    ...
    MainData.GeneralData.forEach(Receipt => {
        ...
        if (Receipt.DocType.includes(Status)) {
            ...
            if (Receipt.DocType === "تأجير" || Receipt.DocType === "مرتجع استئجأر") { Debit = 0; Credit = Receipt.Total }
            ...
            if (Title.includes("استئجأر")) { SumDebit += Debit; SumCredit += Credit }
            if (Title.includes("تأجير")) { SumDebit += Credit; SumCredit += Debit }
        }
    })

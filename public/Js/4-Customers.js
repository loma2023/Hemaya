...
function NewCollectOrPayment(id) {
    ...
    let DocType = Form.querySelector(".DocType")

    DocType.classList.remove("Required");
    DocDate.classList.remove("Required");
    Total.classList.remove("Required");

    if (DocType.value == 'empty') { DocType.classList.add("Required"); return Toast(id = 'Notification', txt = "يرجي تحديد نوع السند",) }
    ...
    let Debit = "الصندوق", Credit = "العملاء", SubDebit = "مقبوضات", SubCredit = CustomerID
    if (DocType.value.includes("دفع")) {
        Debit = "العملاء"; Credit = "الصندوق"; SubDebit = CustomerID; SubCredit = "مدفوعات"
    }
    ...
    fetch("/GeneralData", {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            DocNu: DocNu, DocType: DocType.value, DocDate: DocDate.value, Name: CustomerID,
...
function NewTime(id) {
    ...
    fetch(Link, {
        ...
        body: JSON.stringify({
            ...
            DocType: DocType.value, Statment: Statment, Total: Total.value,
        })
    })
        .then((res) => res.json())
        .then((Data) => {
...
function FormCollect(event) {
    ...
    if (btn.classList.contains("bx-dollar")) {
        ...
        TitleForm.innerText = "تاريخ تحصيل او دفع"

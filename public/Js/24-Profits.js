...
function ShowAllReceipts(MainData, Decode) {
    ...
    MainData.GeneralData.forEach(Receipt => {
        ...
        if (MonthReceipt.includes(MonthReceipt.slice(0, 3) + "-" + ThisYear)) {
            if (Receipt.DocType === "تأجير") { IncomeStatement[index].Salse += Receipt.Total }
            if (Receipt.DocType === "مرتجع تأجير") { IncomeStatement[index].Salse -= Receipt.Total }

            if (Receipt.DocType === "استئجأر") { IncomeStatement[index].Purchases += Receipt.Total }
            if (Receipt.DocType === "مرتجع استئجأر") { IncomeStatement[index].Purchases -= Receipt.Total }

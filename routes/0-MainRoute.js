...
router.get("/MainData.JSON", RequireAuth, async (req, res) => {
  ...
  await HemayaUser.findOne({ _id: Decode.ID })
...
router.get("/SalesReport", RequireAuth, (req, res) => {
  let Decode = jwt.verify(req.cookies.jwt, process.env.JWT_SECRET_KEY);
  res.render("19-SalesReport", { User: Decode, Title: "تقرير التأجير", Buttons: "FALSE" });
});

router.get("/PurchasesReport", RequireAuth, (req, res) => {
  let Decode = jwt.verify(req.cookies.jwt, process.env.JWT_SECRET_KEY);
  res.render("19-SalesReport", { User: Decode, Title: "تقرير الاستئجأر", Buttons: "FALSE" });
});
...
router.put("/ReadNotification:id", RequireAuth, async (req, res) => {
  ...
  await HemayaUser.updateOne({ "NotificationsData._id": req.params.id },
  ...
  try {
    const EmailObj = {
      ...
      <div class="DetailsMsg">
          ...
          <p>الإميل : ${req.body.Email}</p>
          <p>الرسالة : <br> ${req.body.Message} </p>
      </div></div></body></html>`
    };
    let SendMail = await transporter.sendMail(EmailObj)
    if (SendMail) { res.json(Msg.SendMail); }
    else { res.json(Msg.NotSendMail); }

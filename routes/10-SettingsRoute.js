...
router.get("/Settings", RequireAuth, (req, res) => {
    ...
    HemayaUser.findOne({ _id: Decode.ID })
...
router.post("/PersonalDataSettings", RequireAuth, async (req, res) => {
    ...
    try {
        if (Decode.TypeUser === "Owner") {
            await HemayaUser.updateOne({ _id: Decode.ID },
                {
        ...
        if (Decode.TypeUser === "User") {
            await HemayaUser.updateOne({ "UsersData._id": Decode.UserID },
                {
...
router.post("/EmailSettings", RequireAuth, async (req, res) => {
    ...
    let isCurrentEmail;
    try {
        isCurrentEmail = await HemayaUser.findOne({ Email: req.body.Email });
        if (isCurrentEmail) { return res.json(Msg.ExistEmail); }
        if (!isCurrentEmail) {
            isCurrentEmail = await HemayaUser.findOne({ "UsersData.Email": req.body.Email })
            if (isCurrentEmail) { return res.json(Msg.ExistEmail); }
        }
